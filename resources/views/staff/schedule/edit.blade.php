...
@section('content')
    <div class="container my-5">
        <form method="POST" action="{{ route('staff.staff.schedules.update', $schedule['id']) }}">
            @csrf
            @method('PATCH')
            <div class="mb-3">
    ...
    <script>
        function addInput() {
            let content = `<div class="d-flex align items-center hour-additional">
                <input type="time" name="hours[]" id="hours" class="form-control my-2">
                <i class="fa-solid fa-circle-xmark text-danger mx-2" style="font-size: 1.5rem; cursor: pointer;" onlick="this.closest('.hour-additional').remove()"></i>
                </div>`;
            document.getElementById('addtionalInput').innerHTML += content;
        }
    </script>
@endpush
