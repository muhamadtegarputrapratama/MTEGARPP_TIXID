...
@section('content')
    <div class="container my-3">
        ...
        <table class="table table-bordered">
            ...
            @foreach ($movieTrash as $key => $data)
                <tr>
                    <td>{{ $key + 1 }}</td>
                    <td>{{ $schedule['cinema']['name'] ?? ''}}</td>
                    <td>{{ $schedule['movie']['title'] ?? ''}}</td>
                    <td class="d-flex">
                        <form action="{{route('admin.movies.restore', $data->id)}}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button class="btn btn-secondary ms-2">Kembalikan</button>
                        </form>
                           <form action="{{route('admin.users.delete_permanent', $data->id)}}" method="POST">
                            @csrf
                            @method('DELETE')
